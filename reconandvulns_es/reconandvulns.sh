 	...
 	if [[ $1 != http?(s)://* ]]; then
		...
		if [[ ${TELEAPI} = "XXXXXX" ]] || [[ ${CHATID} = "XXXXXX" ]] || [[ ${BXSS} = "XXXXXX.xss.ht" ]]; then
			...
			funcion_descargojs () {
				...
				if [ $capacidad0 == 0 ]; then
					...
					if [ ${TIENEJS} -ne 0 ]; then
						cat ${MIDIR}/${DOMINIO}/js/${DOMINIO}_hakcheckurl_final.txt | xargs -P40 -n1 -I{} sh -c "wget -q --no-check-certificate {}" >/dev/null
			...
			funcion_analizojs () {
				...
				python3 ~/tools/LinkFinder/linkfinder.py -i "${MIDIR}/${DOMINIO}/js/js/*.js" -o ${MIDIR}/${DOMINIO}/linkFinder/linkFinder.html >/dev/null 2>/dev/null
				echo "$(tput setab 2)   [-] [OK]$(tput sgr 0)"
			}
